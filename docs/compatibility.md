...
| Command-hook invocation | N/A | String shell or exec `args` | String shell or exec `args` | String `command` runs via `/bin/sh -lc` (Windows Job supervisor). `args` or a `command` array spawn the executable without a shell. Plugin install is trusted code execution. |
